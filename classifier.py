...
train_data = (pd.DataFrame({'VisitNumber': s['VisitNumber'].apply(float), 'Weekday': s['Weekday'].apply(float),
                            # Create a pandas dataframe with all features as floats(for compatibility to preprocess data and use with SGD Classifier
                            'Upc': s['Upc'].apply(float), 'ScanCount': s['ScanCount'].apply(float),
                            'DepartmentDescription': s['DepartmentDescription'].apply(float),
                            'FinelineNumber': s['FinelineNumber'].apply(float)}))

labels_data = pd.DataFrame({'TripType': s['TripType'].apply(
        float)})  # Make a pandas dataframe with just Trip Type labels from the training data

test_data = pd.DataFrame({'VisitNumber': d['VisitNumber'].apply(float), 'Weekday': d['Weekday'].apply(float),
                          # Create a pandas dataframe with all features as floats(for compatibility to preprocess data and use with SGD Classifier
                          'Upc': d['Upc'].apply(float), 'ScanCount': d['ScanCount'].apply(float),
                          'DepartmentDescription': d['DepartmentDescription'].apply(float),
...
plt.bar(range(len(e)), e.values(),
        align='center')  # Plot the frequencies of predicted labels from the test data using matplotlib
